...
export default function Aluno({ match }) {
    ...
    const handleSubmit = async e => {
        ...
        if (!isFloat(String(peso))) {
            toast.error('Peso precisa ser um número inteiro.');
            formErrors = true
        }

        if (!isFloat(String(altura))) {
            toast.error('Altura precisa ser um número decimal. Ex.: 1.72');
            formErrors = true
        }
        ...
        try {
            ...
            const status = get(err2, 'response.status', 0);
            const data = get(err2, 'response.data', {});
            const errors = get(data, 'errros', []);
    ...
    return (
        <Container>
            ...
            <Form onSubmit={handleSubmit}>
                ...
                Altura:
                <input type="text" value={altura} onChange={e => setAltura(e.target.value)} placeholder="Altura, ex.: 1.72" />
